...
$get_line = $bdd->query('SELECT * FROM produits WHERE product_type = "maillot"');

if (isset($_POST['filter_radio']) && $_POST['filter_radio'] == 'club_value') {
	if (isset($_POST['club']) && $_POST['club'] == 'all') {
		$get_line = $bdd->query('SELECT * FROM produits WHERE club IS NOT NULL AND product_type = "maillot"');
	}
	else if (isset($_POST['club']) && $_POST['club'] != NULL) {
		$get_line = $bdd->query('SELECT * FROM produits WHERE club ="' . $_POST['club'] . '" AND product_type = "maillot"');
	}
}
else if (isset($_POST['filter_radio']) && $_POST['filter_radio'] == 'nation_value') {
	if (isset($_POST['nation']) && $_POST['nation'] == 'all') {
		$get_line = $bdd->query('SELECT * FROM produits WHERE nation IS NOT NULL AND product_type = "maillot"');
	}
	else if (isset($_POST['nation']) && $_POST['nation'] != NULL) {
		$get_line = $bdd->query('SELECT * FROM produits WHERE nation ="' . $_POST['nation'] . '" AND product_type = "maillot"');
	}
}
?>
...
<body>
	...
	<h1 id="title_page">Nos Maillots</h1>
	<div id="filter">
		<h1>Filtres</h1>
